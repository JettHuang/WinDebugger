...
// Windows的虚拟终端引入的目的: 
// 1. 为了兼容过去的Console App, 这些程序使用了Console API设置了控制台属性;
// 2. 在linux/unix系统上，TTY终端使用的都是转义序列进行操作TTY的属性, 所以Win Console要支持这些.
//
//        Process A(Console-APP)   |    ---attach---   PTY终端对象    ---pipe--  |  Process B(一个GUI普通程序模拟一个终端)
//_________________________________|_____________________________________________|________________________________________
//
//           out 字符串                              ------------->                 in字符串
//           Console API设置颜色、光标等.    ---->   生成转义序列      ------>      收到转义序列并执行
//		     in 字符串                       <----   处理转义序列      <-----       获取用户输入(设置颜色、普通文本命令 ...)
//
//
